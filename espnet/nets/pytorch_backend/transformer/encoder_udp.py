...
from espnet.nets.pytorch_backend.backbones.conv3d_extractor_udp import Conv3dResNet_udp
...
class Encoder_udp(torch.nn.Module):
    ...
    def __init__(
        ...
        super(Encoder_udp, self).__init__()
        ...
        if input_layer == "conv1d":
            ...
            self.frontend = Conv3dResNet_udp(relu_type=relu_type)
        else:
            self.frontend = None
        # -- backend module.
    ...
    def forward(self, xs, masks, udps, prompt=None):
        ...
        """
        if isinstance(self.frontend, (Conv1dResNet, Conv3dResNet_udp)):
            xs = self.frontend(xs, udps)
        if prompt != None:
    ...
    def forward_one_step(self, xs, masks, udps, prompt=None, cache=None):
        """Encode input frame.
