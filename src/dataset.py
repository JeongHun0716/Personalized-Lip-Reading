...
class Conformer_LLM_dataset(FairseqDataset):
    def __init__(
            self,
            manifest_path: str,
            sample_rate: float,
    ...
    def __getitem__(self, index):
        ...
        txt_feats = torch.cat((torch.tensor([self.llm_tokenizer.bos_token_id]).long(), txt_feats))
        return {"id": index, 'fid': fid, "video_source": video_feats, 'audio_source': audio_feats, "label_list": labels, 'text_source':[txt_feats]}
    ...
    def collater(self, samples):
        ...
        source = {"audio": collated_audios, "video": collated_videos, "text": collated_texts[0][0]}
