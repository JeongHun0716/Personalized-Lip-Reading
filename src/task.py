...
if DBG:
    from src.dataset import Conformer_LLM_dataset
else:
    from .dataset import Conformer_LLM_dataset

logger = logging.getLogger(__name__)

@dataclass
class Conformer_LLM_TrainingConfig(FairseqDataclass):
    data: str = field(
...
@register_task("conformer_llm_training", dataclass=Conformer_LLM_TrainingConfig)
class Conformer_LLM_TrainingTask(FairseqTask):
    cfg: Conformer_LLM_TrainingConfig
    def __init__(
        self,
        cfg: Conformer_LLM_TrainingConfig,
    ) -> None:
        ...
        logger.info(f"Conformer_LLM_TrainingTask Config {cfg}")
    ...
    @classmethod
    def setup_task(
        cls, cfg: Conformer_LLM_TrainingConfig, **kwargs
    ) -> "Conformer_LLM_TrainingTask":
        if cfg.pdb:
            import pdb
            pdb.set_trace()
        return cls(cfg)
    ...
    def load_dataset(self, split: str, **kwargs) -> None:
        ...
        self.datasets[split] = Conformer_LLM_dataset(
            manifest,
